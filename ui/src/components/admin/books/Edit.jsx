...
import { getBook, saveBook } from '../../../store/modules/books';
...
class Edit extends FormComponent {
    ...
    populateBook() {
        const { history, match, getBook } = this.props;
        const { id } = match.params;

        getBook(id)
            .then((book) => {
                this.setState({ form: this.mapViewToModel(book) });
                    ...
                    <Col lg={8} md="auto">
                        <Form onSubmit={this.handleSubmit} autoComplete="off">
                            ...
                            <Form.Group>
                                ...
                                <Select
                                    name="authors"
                                    value={this.authorOptions().filter(
                                        (option) => {
                                            const { value } = option;

                                            return form['authors'].includes(
                                                value
                                            );
                                        }
                                    )}
                            ...
                            <Form.Group>
                                ...
                                <Select
                                    ...
                                    value={this.categoryOptions().filter(
                                        (option) => {
                                            const { value } = option;

                                            return form['categories'].includes(
                                                value
                                            );
                                        }
                                    )}
...
export default connect(null, { getAuthors, getCategories, getBook, saveBook })(
    Edit
);
